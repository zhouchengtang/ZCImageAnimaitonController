...
@interface ZCImageAnimationController()
{
...
@implementation ZCImageAnimationController
...
{//开始动画
    ...
    if (!_animationWindow && !_animationView) {
        ...
        ZCAnimaitonViewController * viewController = [[ZCAnimaitonViewController alloc] init];
        viewController.vcShouldAutorotate = self.shouldAutorotate;
        viewController.vcSupportedInterfaceOrientations = self.supportedInterfaceOrientations;
        _animationWindow.rootViewController = viewController;
    }
...
{
    if (_isAnimation) {
        __weak ZCImageAnimationController * waekSelf = self;
